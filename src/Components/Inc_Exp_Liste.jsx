function Inc_Exp_Liste({ income, expense, removeIncome, removeExpense }) {
  return (
                          ...
                          {income.map((inc) => (
                              <li key={inc.id} className="list-group-item list-group-item-action list-group-item-info">
                                  <div className="d-flex w-100 justify-content-between">
                                      ...
                                      <span className="badge text-bg-danger rounded-pill" onClick={() => removeIncome(inc.id)}>
                          ...
                          {expense.map((exp) => (
                              <li key={exp.id} className="list-group-item list-group-item-action list-group-item-info">
                                  <div className="d-flex w-100 justify-content-between">
                                      ...
                                      <span className="badge text-bg-danger rounded-pill" onClick={() => removeExpense(exp.id)}>
                                          <i className="bi bi-bag-x"></i>
                                      </span>
                                  </div>
                              </li>
